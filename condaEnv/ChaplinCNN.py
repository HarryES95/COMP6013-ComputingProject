...
def formatAndSaveNumpyData(numpy_video,scene_list,split):
    ...
    for scene in scene_list:
        ...
        while not count >= limit and three is not None: 
            ...
            #pad
            label,sample = np.pad(label,((0,0),(0,0),(hTPad,hBPad),(wLPad,wRPad)),mode="empty"),np.pad(sample,((0,0),(0,0),(hTPad,hBPad),(wLPad,wRPad)),mode="empty")
...
def enhance(numpy_video, model, split):
    writer = svio.FFmpegWriter("E:\\Results\\enhanced.mp4", inputdict={
      '-r': "50",
    },
    outputdict={
      '-vcodec': 'libx264',
    ...
    while two is not None:
        ...
        #pad
        quadrants = np.pad(quadrants,( (0,0),(0,0),(hTPad,hBPad),(wRPad,wLPad)),mode="empty")
...
size = 503140
videoFilePath = "Dataset\\output.mp4"
